...
class RelatorioProjeto_ExtratoMensalController extends Base_Controller_Action
{
    ...
    {
        $this->view->headTitle(Base_Util::setTitle('L_TIT_REL_EXTRATO_MENSAL'));
        $this->initView();
	...
	public function generateAction()
    {
		...
		$this->arrExtratoMensal = $objExtratoMensal->getExtratoMensal($arrParans);
		
		$this->mesAno           = $descMes."/".$arrParans['ano'];

		$arrContrato	    = $this->objContrato->getContratoPorTipoDeObjeto(false, 'P', $arrParans['cd_contrato'], false);
	...
	private function gerarRelatorio(){
        ...
        $this->objPdf->iniciaRelatorio(Base_Util::getTranslator('L_TIT_REL_EXTRATO_MENSAL'), K_CREATOR_SYSTEM.', '.
                                                                                             Base_Util::getTranslator('L_TIT_REL_EXTRATO_MENSAL').', '.
                                                                                             Base_Util::getTranslator('L_VIEW_RELATORIO').', '.
                                                                                             Base_Util::getTranslator('L_VIEW_EXTRATO_MENSAL').', '.
		...
		// print colored table
		$this->montarelatorio($header);

		//Close and output PDF document
		$this->objPdf->Output('relatorio_custo_projeto.pdf', 'I');
	}

	private function montarelatorio($header)
    {
		...
		if( count($this->arrExtratoMensal) > 0 && count($this->arrResumoExtratoMensal) > 0){
			...
			foreach($this->arrExtratoMensal as $row){
				...
				$this->objPdf->Cell($w[0], 6, "     ".Base_Util::getTranslator('L_VIEW_PARCELA_NR')." ".$row['ni_parcela'], 'LRBT', 0, 'L');
				$this->objPdf->Cell($w[1], 6, $row['ni_horas_parcela'].' '.$arrDadosUnidadeMetricaPadrao['tx_sigla_unidade_metrica'], 'LRBT', 1, 'C');
