...
class RelatorioProjeto_ChecklistPropostaController extends Base_Controller_Action
{
	private $arrChecklistProposta;
	private $objPdf;
	private $mesAno;

    public function indexAction()
    {
        $this->view->headTitle(Base_Util::setTitle('L_TIT_REL_CHECKLIST_PROPOSTA'));
    ...
    public function generateAction()
    {
        ...
        $objChecklistProposta		= new RelatorioProjetoChecklistProposta();
        ...
        $this->arrChecklistProposta = $objChecklistProposta->getChecklistProposta($arrParam);
	...
	private function gerarRelatorio(){
        ...
        $arrKeywords = array(K_CREATOR_SYSTEM,
                             Base_Util::getTranslator('L_TIT_REL_CHECKLIST_PROPOSTA'),
                             Base_Util::getTranslator('L_VIEW_RELATORIO'));

        $this->objPdf->iniciaRelatorio(Base_Util::getTranslator('L_TIT_REL_CHECKLIST_PROPOSTA'), $arrKeywords);
		...
		if(count($this->arrChecklistProposta) > 0){
			//Titulos da Coluna
			$header = array(
		...
		$this->objPdf->Output('relatorio_checklist_proposta.pdf', 'I');
	}
	...
	private function montaGridChecklist($header){
		...
		$cd_projeto_aux = 0;
		foreach($this->arrChecklistProposta as $row){
			...
			$this->objPdf->Cell($w[0], 6, "    ".Base_Util::getTranslator('L_VIEW_PROPOSTA_NR')." ".$row['cd_proposta'], 'LR', 0, 'L', $fill);
			$this->objPdf->Cell($w[1], 6, number_format($row['ni_horas_proposta'],1,',','.')." ".$arrDadosMetricaPadrao['tx_sigla_unidade_metrica'], 'LR', 0, 'C', $fill);
			$this->objPdf->Cell($w[3], 6, (!is_null($row['st_fechamento_proposta'     ]))? "OK" : Base_Util::getTranslator('L_VIEW_PENDENTE'), 'LR', 0, 'C', $fill);
			$this->objPdf->Cell($w[4], 6, (!is_null($row['st_parecer_tecnico_proposta']))? "OK" : Base_Util::getTranslator('L_VIEW_PENDENTE'), 'LR', 0, 'C', $fill);
			$this->objPdf->Cell($w[5], 6, (!is_null($row['st_aceite_proposta'         ]))? "OK" : Base_Util::getTranslator('L_VIEW_PENDENTE'), 'LR', 0, 'C', $fill);
			$this->objPdf->Cell($w[6], 6, (!is_null($row['st_homologacao_proposta'    ]))? "OK" : Base_Util::getTranslator('L_VIEW_PENDENTE'), 'LR', 0, 'C', $fill);
			$this->objPdf->Cell($w[2], 6, (!is_null($row['st_alocacao_proposta'       ]))? "OK" : Base_Util::getTranslator('L_VIEW_PENDENTE'), 'LR', 0, 'C', $fill);
			$this->objPdf->Ln();
