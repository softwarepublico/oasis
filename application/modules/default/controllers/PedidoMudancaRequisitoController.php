...
class PedidoMudancaRequisitoController extends Base_Controller_Action
{
	...
	public function gridMudancaRequisitoAction()
	{
		$this->_helper->layout->disableLayout();
		
		$cd_projeto = $this->_request->getParam('cd_projeto');
		
		$res			  = $this->objRequisito->getRequisito($cd_projeto);

		$arrPedidoMudanca = $this->_objGerenciaMudanca->buscaPedidoMudancaEmAndamento($cd_projeto, ItemControleBaselineController::K_ITEM_CONTROLE_BASELINE_REQUISITO);

		if (count($arrPedidoMudanca)) {
			foreach ($arrPedidoMudanca as $pedido){
				foreach ($res as $key => $value){
					if ($value["cd_requisito"] == $pedido["cd_item_controlado"]) {
