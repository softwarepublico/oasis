...
class GerenciamentoTesteRequisitoHomologacaoController extends Base_Controller_Action
{
	...
	public function gridGerenciamentoTesteRequisitoHomologacaoAction()
	{
        ...
        $this->view->res = $this->itemTesteRequisito->getGridItemTesteRequisitoAll('homologacao',$cd_requisito,$cd_projeto);
    }
    ...
    public function saveAction()
    {
        ...
        foreach($_POST as $k=>$v){
            ...
            if( substr($k,0,$tamTx) == $valTx ){
                $homologacoes[substr($k,$tamTx)] = $v;
            }
        }
        $qtdHomologacoes = count($homologacoes);
        $qtdCodigoItens  = count($codigos_itens);
        if( $qtdHomologacoes != $qtdCodigoItens ){
        ...
        foreach( $homologacoes as $codigoItemTeste => $textArea ){
            if( $retorno ){
                $textArea    = trim($textArea);
                $dtRequisito = $this->requisito->getUltimaVersaoRequisito( $cd_projeto,$cd_requisito );
                $arr         = array();
                $arr['cd_item_teste']       = $codigoItemTeste;
                $arr['tx_homologacao']      = $textArea;
                $arr['dt_versao_requisito'] = $dtRequisito[0]['dt_versao_requisito'];
                
                if(!empty($textArea)){
                    $arr['st_item_teste_requisito'] = "F";

                    $arr['dt_homologacao']              = date('Y-m-d');
                    $arr['st_homologacao']              = "S";
                    $arr['cd_profissional_homologacao'] = $_SESSION['oasis_logged'][0]['cd_profissional'];
                } else {
                    $arr['st_item_teste_requisito']     = "H";

                    $arr['dt_homologacao']              = null;
                    $arr['st_homologacao']              = null;
                    $arr['cd_profissional_homologacao'] = null;
                }
    ...
    public function notApproveAction()
    {
        ...
        foreach( $homologacoes as $codigoItemTeste => $textArea ){
            if( $retorno ){
                ...
                $arr = array();
                $arr['tx_homologacao']                  = $textArea;
                $arr['st_item_teste_requisito']         = null;
                if(!empty($textArea)){
                    $arr['dt_homologacao']              = null;
                    $arr['st_homologacao']              = null;
                    $arr['cd_profissional_homologacao'] = null;
