...
class FechamentoExtratoMensalDemandaController extends Base_Controller_Action
{
	private $contrato;
	private $parcela;
	...
	public function init()
	{
		...
		$this->custoDemandaMetrica         = new CustoContratoDemandaPorMetrica($this->_request->getControllerName());
	...
	{
		$this->view->headTitle(Base_Util::setTitle('L_TIT_FECHAMENTO_EXTRATO_MENSAL_DEMANDA'));
	...
	public function gridFechamentoExtratoMensalDemandaAction()
	{
        ...
        $formData = array();
        $formData['ni_mes_custo_metrica']     = $mes_inicial;
        $formData['ni_ano_custo_metrica']     = $ano_inicial;
        $formData['cd_contrato_custo_metrica']= $cd_contrato;
		...
		for ($i=1;$i<=$qtd_meses_contrato;$i++){
			 ...
			 $arrCustoDemandaNoMes = $this->custoDemandaMetrica->getCustoMetrica($formData);

             
             foreach ($arrCustoDemandaNoMes as $parcela){
			 	
                $horas_parcelas += $parcela['nf_qtd_unidade_metrica']; 
                $qtd_parcelas   += 1;
...
//			 	}
			 ...
			 if ($mes == 13){
				$mes = 1;
				$ano = $ano + 1;
                $formData['ni_ano_custo_metrica']     = $ano;
			 }
             $formData['ni_mes_custo_metrica']     = $mes;
		}
		...
		echo $this->view->render('fechamento-extrato-mensal-demanda/grid-fechamento-extrato-mensal-demanda.phtml');
	}
	
	public function fecharExtratoMensalDemandaAction()
	{
		$this->_helper->viewRenderer->setNoRender(true);
		$this->_helper->layout->disableLayout();
	...
	public function gravaExtratoMensalDemanda($cd_contrato, $ni_mes_extrato, $ni_ano_extrato, $qtd_parcelas, $horas_parcelas)
	{
		$erros = false;
