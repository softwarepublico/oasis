...
class PedidoMudancaPropostaController extends Base_Controller_Action
{
	...
	public function gridMudancaPropostaAction()
	{
		$this->_helper->layout->disableLayout();
		
		$cd_projeto = $this->_request->getParam('cd_projeto');
		
		$res			  = $this->_objProposta->getPropostasAlteracao($cd_projeto);

		$arrPedidoMudanca = $this->_objGerenciaMudanca->buscaPedidoMudancaEmAndamento($cd_projeto, ItemControleBaselineController::K_ITEM_CONTROLE_BASELINE_PROPOSTA);

		if (count($arrPedidoMudanca)) {
			foreach ($arrPedidoMudanca as $pedido)
			{
				foreach ($res as $key => $proposta)
				{
					if ($proposta["cd_proposta"] == $pedido["cd_item_controlado"]) {
